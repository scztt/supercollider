...
// SC_COND_ASSERT behaves like SC_ASSERT except that it is always executed, and returns the boolean value of it's expression.
// 
#if defined(BOOST_DISABLE_ASSERTS) || ( defined(BOOST_ENABLE_ASSERT_DEBUG_HANDLER) && defined(NDEBUG) )
	#define SC_COND_ASSERT(expr) (expr)
#else
	#define SC_COND_ASSERT(expr) ((expr) ? (true) : (::boost::assertion_failed(#expr, BOOST_CURRENT_FUNCTION, __FILE__, __LINE__), false))
#endif

#ifdef _MSC_VER
	#define SC_BREAK { __debugbreak(); }
#else
	#define SC_BREAK { raise(SIGTRAP); }
#endif


namespace boost {
  void assertion_failed(char const * expr, char const * function, char const * file, long line) ANALYZER_NORETURN;
  void assertion_failed_msg(char const * expr, char const * msg, char const * function, char const * file, long line) ANALYZER_NORETURN;
}
