...
public:
	...
	DebugTableEntry()
		: mLine(DEBUGTABLE_NOVALUE), mCharacter(DEBUGTABLE_NOVALUE)
	{}
	
	DebugTableEntry(std::uint32_t line, std::uint32_t character)
		: mLine(line), mCharacter(character)
	{}
	
	ValueT							getLine() const
												{ return mLine; }
	
	ValueT							getCharacter() const
												{ return mCharacter; }
...
public:
	...
	const std::deque<DebugTableEntry>&
											entries() const;
